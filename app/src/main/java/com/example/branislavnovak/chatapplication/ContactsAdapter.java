...
public class ContactsAdapter extends BaseAdapter {
    ...
    */
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(contact.getmFirstName().length() > 0) {
            holder.firstLetter.setText(contact.getmFirstName().substring(0, 1).toUpperCase());      // first name letter
        }
        holder.firstLetter.setBackgroundColor(getRandomColor());

        String fullContactName = contact.getmFirstName() + " " + contact.getmLastName();            // making full contact name on contact list
        holder.name.setText(fullContactName);
